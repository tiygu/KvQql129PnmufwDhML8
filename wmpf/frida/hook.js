...
const patchConditionalBranch = (address, expectedBytes, label) => {
    if (!validateTarget(address, expectedBytes, label)) return false;
    const originalBytes = Array.from(new Uint8Array(address.readByteArray(expectedBytes.length)));
    ...
    sendDiag(`${label} branch patched bytes=${originalBytes.length}`);
    return true;
};

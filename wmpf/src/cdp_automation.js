...
const TOTAL_AUTOMATION_TIMEOUT_MS = 30000;
const JSCONTEXT_WAIT_BUDGET_MS = 5000;
...
const selectExecutionContext = async (client, preferredContextName, logger, timeoutMs = CONTEXT_WAIT_TIMEOUT_MS) => {
    ...
    while (Date.now() - startedAt < timeoutMs) {
        ...
        try {
            ...
            logger.main_debug("[auto] Runtime.enable produced no execution context; waiting for jscontext bridge");
...
const waitForInjectedApiReady = async (client, contextId, timeoutMs = INJECTED_API_READY_TIMEOUT_MS, requireOneClick = false) => {
    const startedAt = Date.now();
    while (Date.now() - startedAt < timeoutMs) {
        const state = (await evaluateExpression(client, `(() => {
                ...
                try {
                    const manager = typeof ctl.findOneClickManager === "function"
                        ? ctl.findOneClickManager()
                        : null;
                    oneClickReady = !!manager && typeof manager.onButtonClick === "function";
                } catch (_) {}
        ...
        if (state?.ready && (!requireOneClick || state.oneClickReady)) {
            return state;
        }
        await new Promise((resolve) => setTimeout(resolve, RUNTIME_READY_POLL_MS));
    }
    throw new Error(requireOneClick
        ? `[auto] injected gameCtl farm root became ready, but one-click manager was not loaded within ${timeoutMs}ms`
        : `[auto] injected gameCtl never reported a farm root within ${timeoutMs}ms`);
};
...
const runAutomation = async (client, options, logger, deadlineAt) => {
    ...
    if (options.autoInjectScript) {
        ...
        if (effectiveExpression.includes("gameCtl")) {
            injectedApiState = await waitForInjectedApiReady(client, context.id, getDeadlineRemainingMs(deadlineAt), farmMode);
        }
...
exports.waitForInjectedApiReady = waitForInjectedApiReady;

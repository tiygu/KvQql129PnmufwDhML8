...
class CdpProxyContextReplay {
  constructor(options) {
    ...
    this.isBackendAvailable = options.isBackendAvailable || (() => true);
  ...
  addClient(client) {
    ...
    state.onMessage = (message) => {
      ...
      if (!this.isBackendAvailable()) {
        this._sendBackendDisconnected(client, parsed?.id);
        return;
      }
      ...
      if (typeof parsed?.id === "number") {
        ...
        this.pendingCommands.set(proxyId, { client, originalId: parsed.id });
        forwarded = JSON.stringify({ ...parsed, id: proxyId });
      }
      this.forward(forwarded);
  ...
  removeClient(client) {
    ...
    for (const [proxyId, pending] of this.pendingCommands) {
      if (pending.client === client) this.pendingCommands.delete(proxyId);
    }
  }
  ...
  resetBackend() {
    ...
    for (const pending of this.pendingCommands.values()) {
      this._sendBackendDisconnected(pending.client, pending.originalId);
    }
  ...
  _sendBackendDisconnected(client, id) {
    if (typeof id !== "number" || !this.isClientOpen(client)) return;
    client.send(JSON.stringify({
      id,
      error: {
        code: -32000,
        message: "Miniapp backend disconnected",
      },
    }));
  }
  ...
  handleBackendMessage(message) {
    ...
    if (typeof parsed?.id === "number") {
      ...
      if (this.isClientOpen(pending.client)) {
        pending.client.send(JSON.stringify({ ...parsed, id: pending.originalId }));
      }
      return;
    }

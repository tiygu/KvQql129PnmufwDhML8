...
const {
	...
	reconstructIcon,
	readCdpResource,
	resolveCocosSpriteFrame,
} = require("../src/icon-evidence");
...
test("fixed PNG atlas fixtures reconstruct normal, rotated, offset, and transparent-padding icons", () => {
	...
	const ordinary = reconstructIcon(atlas, {
		mimeType: "image/png",
	...
	assert.deepEqual(pixels(ordinary), {
		width: 2,
		height: 1,
		pixels: { "0,0": COLORS.red, "1,0": COLORS.green },
	});

	const rotated = reconstructIcon(atlas, {
		mimeType: "image/png",
	...
	assert.deepEqual(pixels(rotated), {
		width: 2,
		height: 1,
		pixels: { "0,0": COLORS.red, "1,0": COLORS.green },
	});
	...
	const offset = reconstructIcon(atlas, {
		mimeType: "image/png",
...
test("bottom-left atlas coordinates convert before PNG cropping", () => {
	...
	const icon = reconstructIcon(atlas, {
		mimeType: "image/png",
...
test("CDP reads loaded HTTP, HTTPS, servicewechat, and wxfile image bodies", async () => {
	const urls = [
		"http://assets.test/a.png",
		"https://assets.test/b.png",
		"servicewechat://game/c.png",
		"wxfile://d.png",
	];
	const calls = [];
	const client = {
		send: async (method, params) => {
			...
			if (method === "Page.getResourceTree")
				return {
					frameTree: {
						frame: { id: "root" },
						resources: urls.map((url) => ({ url, mimeType: "image/png" })),
					},
				};
			if (method === "Page.getResourceContent")
				return {
					content: Buffer.from(params.url).toString("base64"),
					base64Encoded: true,
				};
	...
	assert.equal(
		calls.filter(([method]) => method === "Page.getResourceContent").length,
		4,
	);
});
...
test("Cocos provider uses an atomic scene query and the selected Item Identity resource hint", async () => {
	let expression = "",
		context;
	const client = {
		evaluate: async (source, contextId) => {
			expression = source;
			context = contextId;
			return {
				runtimeIdentifier: "frame",
				textureUuid: "uuid",
				resourceUrl: "wxfile://icon.png",
				rect: { x: 0, y: 0, width: 1, height: 1 },
			};
		},
	};
	const result = await resolveCocosSpriteFrame({
		client,
		contextId: 42,
		itemId: "item-7",
		itemIdentity: { itemId: "item-7", iconResourceIdentifier: "IconRes_7" },
	});
...
function withService(run, { concurrency = 2 } = {}) {
	...
	return Promise.resolve(run({ database, dir, concurrency })).finally(() => {
		database.close();
		fs.rmSync(dir, { recursive: true, force: true });
	});
}

test("resource and crop cache persists complete source metadata without duplicate processing", async () =>
	withService(async ({ database, dir }) => {
		let resourceReads = 0;
		const atlas = png(2, 1, { "0,0": "red", "1,0": "green" });
		const events = [];
		const service = new IconEvidenceService({
			database,
			cacheDir: path.join(dir, "icon-cache"),
			concurrency: 1,
			resolveSpriteFrame: async ({ itemId }) => ({
				runtimeIdentifier: `sprite:${itemId}`,
				textureUuid: "texture-uuid",
				resourceUrl: "wxfile://atlas.png",
				mimeType: "image/png",
				rect: { x: itemId === "i1" ? 0 : 1, y: 0, width: 1, height: 1 },
				originalSize: { width: 1, height: 1 },
				offset: { x: 0, y: 0 },
				rotated: false,
			}),
		...
		assert.equal(
			events.some(
				(event) =>
					event.type === "icon-acquisition-complete" && event.cached === true,
			),
			true,
		);
		...
		const wrongCandidate = database.saveIconCandidate({
			itemId: "i1",
			cacheKey: "wrong-generic-icon",
			sourceType: "cocos-runtime-resource",
			runtimeIdentifier: "icon",
			crop: { rect: { x: 9, y: 9, width: 1, height: 1 } },
			rankScore: 1,
			asset: {
				hash: crypto.createHash("sha256").update(wrongBody).digest("hex"),
				mimeType: "image/png",
				width: 1,
				height: 1,
				byteSize: wrongBody.length,
				filePath: wrongFile,
			},
		});
		assert.equal(wrongCandidate.runtimeIdentifier, "icon");
		assert.equal(
			database.getSelectedIconCandidate("i1").runtimeIdentifier,
			"sprite:i1",
		);
		service.request("i1", { contextId: 7 });
		await service.waitForIdle();
		...
		const restartedService = new IconEvidenceService({
			...
			readResource: async () => {
				throw new Error("persistent cache should avoid resource read");
			},
		});
		...
		const portableRoot = fs.mkdtempSync(
			path.join(os.tmpdir(), "icon-snapshot-"),
		);
		const restoredDatabase = new AutomationDatabase(
			path.join(portableRoot, "automation.db"),
		);
		try {
		...
		assert.equal(
			identity.completenessGaps.some((gap) => gap.type === "icon-gap"),
			true,
		);
		const queueEntry = database
			.getCatalogReviewQueue()
			.find((entry) => entry.objectId === "i1");
		assert.equal(
			queueEntry?.reasons.some((reason) => reason.type === "icon-gap") ?? false,
			false,
		);
	}));

test("identical normalized icons from different resources deduplicate by content hash", async () =>
	withService(async ({ database, dir }) => {
		...
		const service = new IconEvidenceService({
			...
			resolveSpriteFrame: async ({ itemId }) => ({
				...
				resourceUrl: `https://assets.test/${itemId}.png`,
				mimeType: "image/png",
				rect: { x: 0, y: 0, width: 1, height: 1 },
				originalSize: { width: 1, height: 1 },
				offset: { x: 0, y: 0 },
...
test("identical resource and crop reuse processing across Item Identities", async () =>
	withService(async ({ database, dir }) => {
		let reads = 0;
		const service = new IconEvidenceService({
			database,
			...
			readResource: async () => {
				reads += 1;
				return {
					body: png(1, 1, { "0,0": "blue" }),
					mimeType: "image/png",
					resolvedUrl: "wxfile://shared.png",
				};
			},
		});
		service.request("i1");
		...
		assert.equal(
			database.getSelectedIconCandidate("i1").assetHash,
			database.getSelectedIconCandidate("i2").assetHash,
		);
	}));

test("image tasks are bounded and request returns before processing", async () =>
	withService(async ({ database, dir }) => {
		let runtimeActive = 0,
			runtimeMaximum = 0,
			offlineActive = 0,
		...
		const service = new IconEvidenceService({
			...
			readResource: async () => {
				...
				runtimeActive -= 1;
				return { body: png(1, 1, { "0,0": "yellow" }), mimeType: "image/png" };
			},
		});
		const processImage = service.processImage;
		service.processImage = async (input) => {
			offlineActive += 1;
		...
		const queued = ["i1", "i2", "i3", "i4", "i5", "i6", "i7"].map((itemId) =>
			service.request(itemId),
		);
		assert.equal(
			queued.every((result) => result.status === "queued"),
			true,
		);
		assert.equal(runtimeActive, 0);
		await service.waitForIdle();
		assert.equal(runtimeMaximum, 1);
		assert.equal(offlineMaximum, 2);
	}));

test("image decoding and reconstruction yield the Node event loop to a worker", async () =>
	withService(async ({ database, dir }) => {
		...
		assert.ok(
			turns > 1,
			`expected worker processing to leave the event loop responsive, observed ${turns} turns`,
		);
	}));
...
test("automation priority aborts an in-flight icon CDP task at its current boundary", async () => {
	...
	const service = new IconEvidenceService({
		...
		concurrency: 1,
		isSafeBoundary: () => safe,
		resolveSpriteFrame: ({ signal }) =>
			new Promise((_resolve, reject) =>
				signal.addEventListener(
					"abort",
					() =>
						reject(
							Object.assign(new Error("CDP command aborted"), {
								name: "AbortError",
							}),
						),
					{ once: true },
...
test("Automation Runtime acquires icons in the background without changing Active planning eligibility", async () => {
	...
	fs.writeFileSync(
		path.join(root, "captures", "item-catalog.json"),
		JSON.stringify({
			chains: [
				{
					id: "c",
			...
			items: [
				{ id: "i1", chainId: "c", level: 1, baseUnits: 1, mergeTarget: "i2" },
				{ id: "i2", chainId: "c", level: 2, baseUnits: 2, mergeTarget: null },
			],
			producers: [],
		}),
		"utf8",
	...
	let reads = 0;
	runtime.iconService.resolveSpriteFrame = async ({ itemId }) => ({
		runtimeIdentifier: itemId,
		resourceUrl: "wxfile://icon.png",
		mimeType: "image/png",
		rect: { x: 0, y: 0, width: 1, height: 1 },
	...
	runtime.iconService.readResource = async () => {
		assert.equal(
			runtime.sessionSupervisor.snapshot().boundaryOwner,
			"icon-exact-read",
		);
		reads += 1;
		return {
			body: png(1, 1, { "0,0": "green" }),
			mimeType: "image/png",
			resolvedUrl: "wxfile://icon.png",
		};
	};
	const processImage = runtime.iconService.processImage;
	runtime.iconService.processImage = async (input) => {
		assert.equal(runtime.sessionSupervisor.snapshot().boundaryBusy, false);
		return processImage(input);
	};
	try {
		const planningBefore = runtime
			.getPlanningCatalog()
			.items.map((item) => item.id);
		assert.equal(
			runtime.getCatalogObject("item-identity", "i1").selectedIcon,
			null,
		);
		...
		assert.equal(reads, 1);
		assert.deepEqual(
			runtime.getPlanningCatalog().items.map((item) => item.id),
			planningBefore,
		);
		assert.match(
			runtime.getCatalogObject("item-identity", "i1").selectedIcon.url,
			/^\/api\/catalog\/icon\//,
		);
		assert.deepEqual(
			events
				.filter((event) => event.type === "catalog-display-icon-updated")
		...
		assert.equal(
			runtime.getCatalogView().items.find((item) => item.id === "i1").iconUrl !=
				null,
			true,
		);
		runtime.database.observeCatalogObject({
			objectType: "item-identity",
			objectId: "board-only",
			payload: { itemId: "board-only", level: 5 },
			sourceType: "passive-runtime",
			sourceRef: "board-state",
			countDuplicate: false,
		});
		runtime.queueVisibleBoardIconEvidence({
			board: { grids: [{ itemId: "board-only", level: 5 }] },
		});
		await runtime.iconService.waitForIdle();
		assert.match(
			runtime.getCatalogView().iconUrls["board-only"],
			/^\/api\/catalog\/icon\//,
		);
		assert.equal(
		...
		runtime.sessionSupervisor.adoptSession({
			kind: "bounded",
			sessionId: null,
			abortController: new AbortController(),
		});
		assert.throws(
			() => runtime.acquireCatalogIcon("i2"),
			(error) => error.code === "ICON_ACQUISITION_UNSAFE_BOUNDARY",
		);
		runtime.sessionSupervisor.finish();
		const releaseBoundary = runtime.beginActionBoundary("test-icon-boundary");
		assert.throws(
			() => runtime.acquireCatalogIcon("i2"),
			(error) => error.code === "ICON_ACQUISITION_UNSAFE_BOUNDARY",
		);

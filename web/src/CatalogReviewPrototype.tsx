...
export default function CatalogReviewPrototype() {
  ...
  const choose = (nextIndex: number) => {
    ...
    setIndex(nextIndex); setSelectedName(next.names.find((x: Item["names"][number]) => x.selected)?.value || next.names[0].value); setDraftName(next.name); setEditing(false); setEvidenceOpen(false);
  };
  ...
  const confirm = (forcedIndex?: number) => {
    ...
    if (!reviewed.blocker.missing.length) {
      advance(draftName !== reviewed.name || selectedName !== reviewed.names.find((x: Item["names"][number]) => x.selected)?.value ? "修改后确认" : "确认无误", reviewedIndex);
      return;
    }

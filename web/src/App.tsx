...
export default function App() {
  ...
  const [activeIconHarvestJob, setActiveIconHarvestJob] = useState<{
    mergeChainId: string;
    jobId: string;
    detailUrl: string;
  } | null>(null);
  ...
  const performRefresh = async () => {
    ...
    try {
      ...
      if (next) {
        ...
        const snapshot = next.automationStatus;
        const revision = Number(snapshot?.statusRevision) || 0;
        if (!revision || revision >= statusRevisionRef.current) {
          if (revision) statusRevisionRef.current = revision;
          setData((old: any) => {
  ...
  const continueCatalogCollection = () => { setReviewFocus(null); setTab("overview"); setMessage("已返回自动化，等待正常订单推进收集真实合成结果"); };
  ...
  const selectedChainActiveIconHarvestJob = activeIconHarvestJob?.mergeChainId
    === String(selectedChain?.id || "")
    ? activeIconHarvestJob
    : null;
  const acquireSelectedChainIcons = async () => {
    const mergeChainId = String(selectedChain?.id || "");
    if (!mergeChainId) return;
    if (selectedChainActiveIconHarvestJob) {
      const existing = await controlApi.getIconHarvestJob(
        selectedChainActiveIconHarvestJob.jobId,
      );
      if (!existing.finalStatus) {
        window.open(
          selectedChainActiveIconHarvestJob.detailUrl,
          "_blank",
          "noopener,noreferrer",
        );
        setMessage(`已打开现有采集任务 ${selectedChainActiveIconHarvestJob.jobId}。`);
        return;
      }
      setActiveIconHarvestJob(null);
    }
    setLoading(true);
    setMessage(`正在预检合成链 ${mergeChainId} 的冻结采集范围…`);
    try {
      const preflight = await controlApi.preflightMergeChainIconHarvest(
        mergeChainId,
      );
      if (preflight.duplicates.matchingActiveJob) {
        const matching = preflight.duplicates.matchingActiveJob;
        setActiveIconHarvestJob({ mergeChainId, ...matching });
        window.open(matching.detailUrl, "_blank", "noopener,noreferrer");
        setMessage(`已打开现有采集任务 ${matching.jobId}。`);
        return;
      }
      const members = preflight.frozenMembers
        .map((member) => `${member.order + 1}. ${member.itemId}（${member.level == null ? "等级未知" : `L${member.level}`}）`)
        .join("\n");
      const gaps = preflight.missingIdentities.length
        ? `\n缺失 Item Identity：${preflight.missingIdentities.join("、")}（将保留为待补采项）`
        : "";
      const differentScope = preflight.duplicates.requiresExplicitNewJob
        ? `\n已有 ${preflight.duplicates.differentScopeActiveJobs.length} 个范围不同的活跃任务；确认后将显式创建新任务。`
        : "";
      const capacity = `需要 ${preflight.capacity.required} 个队列槽位，可用 ${preflight.capacity.available} 个`;
      if (!preflight.capacity.admissible) {
        setMessage(`整链采集容量不足：${capacity}，未创建任务。`);
        return;
      }
      const confirmed = window.confirm(
        `确认创建 Merge-Chain Icon Harvest？\n`
        + `合成链：${mergeChainId}\n`
        + `Catalog Query Revision：${preflight.catalogQueryRevision}\n`
        + `冻结成员：${preflight.frozenMembers.length}/${preflight.expectedMemberCount}\n`
        + `${members}${gaps}\n`
        + `采集合约：${preflight.contractChecks.acquisitionContractVersion}（${preflight.contractChecks.status}）\n`
        + `容量：${capacity}\n`
        + `安全状态：${preflight.safety.state}${preflight.safety.reason ? `（${preflight.safety.reason}）` : ""}`
        + differentScope,
      );
      if (!confirmed) {
        setMessage("已取消整链图标采集，未创建任务或占用队列。");
        return;
      }
      const job = await controlApi.createMergeChainIconHarvestJob({
        preflightId: preflight.preflightId,
        mergeChainId,
        createNew: preflight.duplicates.requiresExplicitNewJob,
        idempotencyKey: preflight.preflightId,
      });
      if (!job.finalStatus) {
        setActiveIconHarvestJob({
          mergeChainId,
          jobId: job.jobId,
          detailUrl: `/api/catalog/icon-harvest-jobs/${job.jobId}`,
        });
      }
      setMessage(
        `整链图标采集任务 ${job.jobId} 已原子入队：${job.progress.settled}/${job.progress.total} 已结算。`,
      );
    } catch (error: any) {
      if (error.payload?.existingJobId) {
        const existingJobId = String(error.payload.existingJobId);
        const existing = await controlApi.getIconHarvestJob(existingJobId);
        if (!existing.finalStatus) {
          const detailUrl = `/api/catalog/icon-harvest-jobs/${existingJobId}`;
          setActiveIconHarvestJob({ mergeChainId, jobId: existingJobId, detailUrl });
          window.open(detailUrl, "_blank", "noopener,noreferrer");
          setMessage(`已打开匹配的现有任务 ${existingJobId}。`);
        } else {
          setActiveIconHarvestJob(null);
          setMessage(`匹配的任务 ${existingJobId} 已结算，可重新预检。`);
        }
      } else if (error.status === 429) {
        setMessage(
          `整链采集容量不足：需要 ${error.payload?.required ?? "?"}，可用 ${error.payload?.available ?? "?"}，未创建任务。`,
        );
      } else {
